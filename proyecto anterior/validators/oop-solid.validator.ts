// Validador para Programación Orientada a Objetos y Principios SOLID
...
export function validateOOPPrinciples(payload: OOPValidationPayload) {
  ...
  if (payload.code && payload.code.includes('if') && payload.code.includes('instanceof')) {
   ...
   if (payload.code) {
     ...
     // Feature Envy (difícil detectar automáticamente, pero indicador)
  ...
  if (payload.methodCount && payload.methodCount > 10) {
   ...
   if (payload.code) {
     ...
     if (responsibilityCount > 1) {
       ...
       if (hasDataAccess) responsibilities.push("acceso a datos");
       if (hasBusinessLogic) responsibilities.push("lógica de negocio");
       if (hasPresentation) responsibilities.push("presentación");
       if (hasSecurity) responsibilities.push("seguridad");
       if (hasLogging) responsibilities.push("logging");
...
export function validateCodeMetrics(payload: OOPValidationPayload) {
  ...
  // Complejidad ciclomática
  if (payload.cyclomaticComplexity && payload.cyclomaticComplexity > 10) {
    errors.push("❌ MÉTRICAS: Complejidad ciclomática alta (>10) - simplificar lógica");
  } else if (payload.cyclomaticComplexity && payload.cyclomaticComplexity > 7) {
    warnings.push("⚠️ MÉTRICAS: Complejidad ciclomática moderada (>7)");
  }
  ...
  // Número de métodos
  if (payload.methodCount && payload.methodCount > 15) {
    warnings.push("⚠️ MÉTRICAS: Muchos métodos (>15) - posible God Class");
  }
